...
- 联机时向服务端发送带序号的输入，并对本地玩家移动与开火做即时预测。
- 接收服务端 `snapshot` 后按 `lastProcessedSeq` 清理已确认输入，再重放未确认输入以校正画面。
- Canvas 画面使用 `gameRef` 高频绘制，React 状态只在 HUD 数据变化时更新，减少主线程重渲染压力。
...
- 使用 `nanoid` 创建 6 位房间码。
- 将活跃房间保存在内存中。
- 每 33ms 左右推进一次游戏状态，即约 30Hz。
- 每 66ms 左右广播一次权威 `snapshot`，即约 15Hz。
- 发送快照前检查 WebSocket `bufferedAmount`，客户端积压时跳过旧快照，避免延迟持续增长。
- 普通快照只发送动态实体；进房、重开、重连和墙体变化时才发送完整地图块。
- 消费 `firePressed` 开火事件，避免开火输入被后续未开火输入覆盖。
- 玩家断线时暂停房间。
...
- `seq` 是客户端递增输入序号，用于服务端确认和客户端预测回放。
- `clientTime` 用于客户端收到快照后估算连接延迟。
- `firePressed` 表示一次开火按键事件，服务端每 tick 最多消费一次。
...
`snapshot.game` 是权威游戏状态。普通快照包含玩家、敌军、炮弹、特效、得分、生命、关卡、波次进度和当前状态；当 `full` 为 `true` 时，还会包含完整 `blocks` 地图块。客户端用 `blockVersion` 判断地图是否需要更新。
...
- 服务端以约 30Hz 模拟游戏、约 15Hz 广播快照，前端只预测本地玩家操作，分数、生命、胜负和全局碰撞仍以服务端为准。
- 如果客户端 WebSocket 发送队列积压，服务端会跳过旧快照，只发送后续最新状态。
